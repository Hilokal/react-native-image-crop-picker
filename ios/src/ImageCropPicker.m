...
@interface CustomCropOverlayView : UIView
@property (nonatomic, strong) FLAnimatedImageView *imageView;
@end

@implementation CustomCropOverlayView
                ...
                [phAsset requestContentEditingInputWithOptions:nil completionHandler:^(PHContentEditingInput * _Nullable contentEditingInput, NSDictionary * _Nonnull info) {
                    [manager
                     ...
                     resultHandler:^(NSData *imageData, NSString *dataUTI,
                        ...
                        if(isGif){
                            self.gifFile = [[NSMutableDictionary alloc] init];
                            filePath = [sourceURL absoluteString];

                            self.croppingFile[@"sourceURL"] = sourceURL;

                            self.gifFile[@"isGif"] = @(isGif);
                            self.gifFile[@"imageData"] = imageData;
                            self.gifFile[@"width"] = @(imgT.size.width);
                            self.gifFile[@"height"] = @(imgT.size.height);
                            self.gifFile[@"mime"] = mimeType;
                            self.gifFile[@"length"] = @(imageData.length);
                            ...
                            self.gifFile[@"isGif"] = nil;
                            self.gifFile[@"imageData"] = nil;
                            self.gifFile[@"width"] = nil;
                            self.gifFile[@"height"] =nil;
                            self.gifFile[@"mime"] = nil;
                            self.gifFile[@"length"] = nil;
                        }
...
- (void) processSingleImagePick:(UIImage*)image withExif:(NSDictionary*) exif withViewController:(UIViewController*)viewController withSourceURL:(NSString*)sourceURL withLocalIdentifier:(NSString*)localIdentifier withFilename:(NSString*)filename withCreationDate:(NSDate*)creationDate withModificationDate:(NSDate*)modificationDate
                withGIFanimated:(Boolean)GIFanimated
{
    ...
    NSLog(@"id: %@ filename: %@ sourceURL: %@", localIdentifier, filename, sourceURL);

    NSLog(@"GIFanimated %s", GIFanimated ? "true" : "false");
...
- (void)imageCropViewController:(UIViewController *)controller
    ...
    if (self.gifFile[@"isGif"]){
        NSDictionary* exif = nil;
        if([[self.options objectForKey:@"includeExif"] boolValue]) {
            exif = [[CIImage imageWithData:self.gifFile[@"imageData"]] properties];
        }
        [self dismissCropper:controller selectionDone:YES completion:[self waitAnimationEnd:^{
            self.resolve([self createAttachmentResponse: self.croppingFile[@"sourceURL"]
                                               withExif: exif
                                    ...
                                    withLocalIdentifier: self.croppingFile[@"localIdentifier"]
                                           withFilename: self.croppingFile[@"filename"]
                                              withWidth: self.gifFile[@"width"]
                                             withHeight: self.gifFile[@"height"]
                                               withMime: self.gifFile[@"mime"]
                                               withSize:[NSNumber numberWithUnsignedInteger:[self.gifFile[@"length"] integerValue]]
                                           withDuration: nil
                                               withData:[[self.options objectForKey:@"includeBase64"] boolValue] ? [self.gifFile[@"imageData"] base64EncodedStringWithOptions:0] : nil
                                               withRect:cropRect
...
- (void)cropAnimatedImage:(FLAnimatedImage *)image {
    dispatch_async(dispatch_get_main_queue(), ^{
        FLAnimatedImageView *imageView = [[FLAnimatedImageView alloc] initWithFrame:CGRectZero];
        imageView.contentMode = UIViewContentModeScaleAspectFit;
        imageView.animatedImage = image;

        TOCropViewController *cropVC = [[TOCropViewController alloc] init];

        cropVC.cropView.cropBoxResizeEnabled = NO;
        cropVC.cropView.hidden = YES;

        CGSize imageSize = imageView.image.size;
        CGSize cropViewSize = cropVC.cropView.frame.size;
        CGFloat scale = MAX(imageSize.width / cropViewSize.width, imageSize.height / cropViewSize.height);

        CGFloat width = imageSize.width / scale;
        CGFloat height = imageSize.height / scale;

        CGRect imageViewFrame = CGRectMake((cropViewSize.width - width) / 2.0, (cropViewSize.height - height) / 2.0, width, height);

        // Set the frame of the imageView
        imageView.frame = imageViewFrame;

        [cropVC.view addSubview:imageView];
        ...
        [[self getRootVC] presentViewController:cropVC animated:YES completion:nil];
    });
}
